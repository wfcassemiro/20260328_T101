...
$page_title = 'Acesso gratuito de sete dias - Translators101';
$page_description = 'Crie sua conta e tenha acesso completo por sete dias';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        if (!$phoneValidation['valid']) {
            ...
            $canReceive = canReceiveTrial($pdo, $email, 'trial_7d');
            ...
            if (!$canReceive['can_receive']) {
                ...
                try {
                    ...
                    $stmt = $pdo->prepare("SELECT id, role, had_trial_7d FROM users WHERE email = ?");
                    $stmt->execute([$email]);
                    $existingUser = $stmt->fetch();
                    
                    if ($existingUser) {
                        if ($existingUser['had_trial_7d']) {
                            $error = 'Você já utilizou o acesso de sete dias anteriormente. Para continuar acessando, assine o plano Premium.';
                        } else {
                        ...
                        $stmt = $pdo->prepare("
                            INSERT INTO users (id, name, email, password_hash, whatsapp, role, trial_approval_token, trial_approved, is_active, created_at)
                            VALUES (?, ?, ?, ?, ?, 'trial_7d', ?, 0, 1, NOW())
                        ");
                        ...
                        $emailSubject = "[Trial 7D] Nova solicitação: " . $name;
                        $emailBody = "
                        <h2>Nova solicitação de Acesso de 7 Dias</h2>
                        ...
                        <table style='border-collapse: collapse; margin: 20px 0;'>
                            ...
                            <tr><td style='padding: 8px; border: 1px solid #ddd;'><strong>Tipo:</strong></td><td style='padding: 8px; border: 1px solid #ddd;'>Acesso de 7 Dias</td></tr>
                        ...
                        <p><a href='{$approvalLink}' style='display: inline-block; padding: 12px 24px; background: #10b981; color: white; text-decoration: none; border-radius: 8px; font-weight: bold;'>Clique aqui para aprovar ou rejeitar</a></p>
                    ...
                    error_log("Erro ao criar conta trial_7d: " . $e->getMessage());
...
<div class="main-content">
    <div class="trial-hero">
        ...
        <h1><i class="fas fa-unlock-alt"></i> Acesso completo por sete dias</h1>
        <p>Experimente todo o conteúdo da Translators101 gratuitamente!</p>
        
        <div class="trial-benefits">
            <div class="benefit-item">
                <i class="fas fa-play-circle"></i>
                <span>Todas as palestras e ferramentas</span>
            </div>
            <div class="benefit-item">
    ...
    <div class="video-card trial-form-card">
        <?php if ($success): ?>
            <div class="success-card">
                ...
                <p>Sua solicitação de acesso foi enviada.</p>
                ...
                <p style="font-size: 0.9rem; color: rgba(255,255,255,0.6);">
                    Aguarde a mensagem em seu WhatsApp antes de tentar fazer login!
                </p>
        ...
        <?php else: ?>
            ...
            <form method="POST">
                ...
                <button type="submit" class="submit-btn">
                    <i class="fas fa-rocket"></i> Solicitar meus sete dias grátis
                </button>
                ...
                <div class="info-box">
                    ...
                    <ul>
                        <li>O WhatsApp será usado para envio da liberação do acesso</li>
                        <li>Você receberá a confirmação em até 8 horas</li>
                        <li>O período de sete dias só começará após o seu primeiro login</li>
                        <li>Não é permitido mais de um período gratuito</li>
                    </ul>
                </div>
            </form>
