...
// INCLUIR A CLASSE DE EMAIL (Fix para envio SMTP)
if (file_exists(__DIR__ . '/../email.php')) {
    require_once __DIR__ . '/../email.php';
} elseif (file_exists(__DIR__ . '/email.php')) {
    require_once __DIR__ . '/email.php';
}
...
// Função auxiliar de envio de email
function sendCertificateNotification($user_email, $user_name, $lecture_title, $certificate_id) {
    if (!class_exists('EmailSender')) return false;
    
    $base_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'];
    $download_url = $base_url . "/download_certificate_files.php?id=" . $certificate_id;
    
    $subject = "Seu Certificado - " . $lecture_title;
    $message = "Olá " . htmlspecialchars($user_name) . ",<br><br>";
    $message .= "Seu certificado para a palestra <strong>" . htmlspecialchars($lecture_title) . "</strong> foi gerado com sucesso.<br><br>";
    $message .= "Você pode baixá-lo clicando no link abaixo:<br>";
    $message .= "<a href='" . $download_url . "'>Baixar Certificado</a><br><br>";
    $message .= "Atenciosamente,<br>Equipe Translators101";
    
    $emailSender = new EmailSender();
    return $emailSender->sendEmail($user_email, $user_name, $subject, $message);
}
        ...
        if (!empty($user_ids) && !empty($lecture_id)) {
            try {
                ...
                if ($lecture) {
                    ...
                    foreach ($user_ids as $user_id) {
                        ...
                        if ($user) {
                            ...
                            if (!$stmt->fetch()) {
                                ...
                                // Enviar Email
                                if (sendCertificateNotification($user['email'], $user['name'], $lecture['title'], $cert_uuid)) {
                                    $email_count++;
                                }
                    ...
                    $success_message = "$count certificados gerados em lote. $email_count emails enviados.";
                } else {
