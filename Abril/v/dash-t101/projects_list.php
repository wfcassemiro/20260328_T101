...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        $pid = $_POST['project_id'];
        try {
            // Excluir despesas de interpretação associadas
            $pdo->prepare("DELETE FROM dash_interpretation_expenses WHERE project_id = ? AND user_id = ?")->execute([$pid, $user_id]);
...
// --- Carregar despesas de interpretação por projeto ---
$project_expenses = [];
if (!empty($projects)) {
    ...
    if (!empty($project_ids)) {
        ...
        $stmt_exp = $pdo->prepare("
            SELECT project_id, paid_by, SUM(amount) as total 
            FROM dash_interpretation_expenses 
            WHERE project_id IN ($placeholders) AND user_id = ?
            GROUP BY project_id, paid_by
        ");
        $exp_params = array_merge($project_ids, [$user_id]);
        $stmt_exp->execute($exp_params);
        $exp_rows = $stmt_exp->fetchAll(PDO::FETCH_ASSOC);
        
        foreach ($exp_rows as $row) {
            $pid = $row['project_id'];
            if (!isset($project_expenses[$pid])) {
                $project_expenses[$pid] = ['client' => 0, 'interpreter' => 0];
            }
            $project_expenses[$pid][$row['paid_by']] = (float)$row['total'];
        }
    }
}
...
<style>
    ...
    /* Coluna de Despesas */
    .expense-cell { font-size: 0.85rem; line-height: 1.6; }
...
<div class="main-content">
    ...
    <div class="video-card profile-header-card" style="background: linear-gradient(135deg, var(--brand-purple), #4a148c); border: none; margin-bottom: 25px;">
        <div class="header-icon-container" style="background: rgba(255, 255, 255, 0.1); border-radius: 50%; width: 60px; height: 50px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <i class="fas fa-folder-open" style="font-size: 1.8rem; color: #fff;"></i>
        </div>
        <div class="header-text-container" style="margin-left: 20px;">
            <h2 style="margin: 0 0 5px 0; padding: 0; font-size: 1.5rem; color: #fff; font-weight: 600; border: none;">Meus projetos</h2>
            <p style="margin: 0; color: rgba(255, 255, 255, 0.8); font-size: 1rem;">Gerencie seus trabalhos e faturas</p>
        </div>
    </div>
            ...
            <table class="vision-table">
                ...
                <tbody>
                    <?php if(empty($projects)): ?>
                        <tr><td colspan="7" style="text-align:center; padding:30px; color:#aaa;">Nenhum projeto encontrado.</td></tr>
                    <?php else: ?>
                        <?php foreach ($projects as $p): 
                            $pid = $p['id'];
                            $has_expenses = isset($project_expenses[$pid]);
                            $exp_client = $has_expenses ? ($project_expenses[$pid]['client'] ?? 0) : 0;
                        ...
                        <tr>
                            ...
                            <td class="expense-cell">
                                <?php if ($has_expenses): ?>
                                    ...
                                    <?php endif; ?>
                                    <?php if ($exp_interpreter > 0): ?>
                                        <div class="expense-interpreter"><i class="fas fa-user" style="width:14px;"></i> <?php echo number_format($exp_interpreter, 2, ',', '.'); ?></div>
                                    <?php endif; ?>
                                <?php else: ?>
                                    <span class="expense-none">-</span>
                                <?php endif; ?>
                            </td>
