...
$default_services = ['Tradução', 'Revisão', 'Interpretação', 'Localização', 'Transcrição', 'Legendagem', 'MTPE', 'DTP'];
$default_languages = ['Português', 'Inglês', 'Espanhol', 'Francês', 'Alemão', 'Italiano', 'Chinês', 'Japonês'];
$default_units = ['Palavra', 'Hora', 'Diária', 'Minuto', 'Lauda', 'Projeto', 'Página'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'add' || $action === 'edit') {
            ...
            $phone_country_code = $_POST['phone_country_code'] ?? '+55';
            ...
            if ($action === 'add') {
                $stmt = $pdo->prepare("INSERT INTO dash_freelancers (user_id, name, email, phone, phone_country_code, is_whatsapp, country, currency, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$user_id, $name, $email, $phone, $phone_country_code, $is_whatsapp, $country, $currency, $notes]);
                $freelancer_id = $pdo->lastInsertId();
                $_SESSION['temp_message'] = "Fornecedor adicionado com sucesso!";
            } else {
                $freelancer_id = $_POST['freelancer_id'];
                $stmt = $pdo->prepare("UPDATE dash_freelancers SET name = ?, email = ?, phone = ?, phone_country_code = ?, is_whatsapp = ?, country = ?, currency = ?, notes = ? WHERE id = ? AND user_id = ?");
                $stmt->execute([$name, $email, $phone, $phone_country_code, $is_whatsapp, $country, $currency, $notes, $freelancer_id, $user_id]);
...
<style>
    ...
    /* Campo de Telefone com WhatsApp */
    .phone-wrapper { display: flex; flex-direction: column; gap: 10px; }
    .phone-input-row { display: flex; gap: 10px; }
    .phone-country-select { flex: 0 0 180px; }
    .phone-number-input { flex: 1; }
    .whatsapp-check-wrapper { display: flex; align-items: center; gap: 10px; font-size: 0.9rem; color: #25D366; cursor: pointer; }
    .whatsapp-check-wrapper input { accent-color: #25D366; width: 18px; height: 18px; cursor: pointer; }
    .whatsapp-check-wrapper i { font-size: 1.2rem; }
    ...
    .vision-btn-success { background: #22c55e; }
    .vision-btn-success:hover { background: #16a34a; }
    .report-nav-buttons { display: flex; gap: 15px; margin-bottom: 20px; flex-wrap: wrap; }
...
<div class="main-content">
    ...
    <div class="report-nav-buttons">
        ...
        <?php if ($edit_freelancer): ?>
            <a href="freelancers.php" class="vision-btn vision-btn-success"><i class="fas fa-user-plus"></i> Adicionar novo fornecedor</a>
        <?php endif; ?>
    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="phone-wrapper">
                        <div class="phone-input-row">
                            <select id="phone_country_code" name="phone_country_code" class="vision-input phone-country-select">
                                <?php foreach ($country_codes as $code => $label): ?>
                                    <option value="<?php echo $code; ?>" <?php echo (($edit_freelancer['phone_country_code'] ?? '+55') == $code) ? 'selected' : ''; ?>><?php echo $label; ?></option>
                                <?php endforeach; ?>
                            </select>
                            <input type="text" id="phone" name="phone" class="vision-input phone-number-input" placeholder="(00) 00000-0000" value="<?php echo htmlspecialchars($edit_freelancer['phone'] ?? ''); ?>">
                        </div>
                        <label class="whatsapp-check-wrapper">
                            <input type="checkbox" name="is_whatsapp" value="1" <?php echo (!empty($edit_freelancer['is_whatsapp'])) ? 'checked' : ''; ?>>
            ...
            <div class="rates-section">
                ...
                <div id="rates_container">
                    <?php if (!empty($edit_rates)): ?>
                        <?php foreach ($edit_rates as $idx => $rate): 
                            ...
                            <div class="rate-inputs-wrapper">
                                <div class="form-group rate-group-service">
                                    ...
                                    <label class="mono-check-wrapper">
                                        ...
                                        <input type="hidden" name="rates_is_monolingual[<?php echo $idx; ?>]" class="mono-val" value="<?php echo $is_mono ? '1' : '0'; ?>">
                                ...
                                <div class="form-group rate-group-lang group-from" style="<?php echo $is_mono ? 'display:none' : ''; ?>">
                                    ...
                                    <select name="rates_lang_from[]" class="vision-input lang-select">
                                        ...
                                        <?php endforeach; ?>
                                        <option value="other" <?php echo (!in_array($rate['lang_from'], $default_languages) && !$is_mono && !empty($rate['lang_from'])) ? 'selected' : ''; ?>>Outro...</option>
                                    </select>
                                    <input type="text" name="rates_lang_from_other[]" class="vision-input lang-other-input" placeholder="Qual?" style="<?php echo (!in_array($rate['lang_from'], $default_languages) && !$is_mono && !empty($rate['lang_from'])) ? 'display:block;margin-top:5px' : 'display:none'; ?>" value="<?php echo (!in_array($rate['lang_from'], $default_languages) && !empty($rate['lang_from'])) ? htmlspecialchars($rate['lang_from']) : ''; ?>">
                                </div>
    ...
    <div class="rate-row">
        <div class="rate-inputs-wrapper">
            <div class="form-group rate-group-service">
                ...
                <label class="mono-check-wrapper">
                    <input type="checkbox" class="mono-check" name="rates_is_monolingual_chk[]">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let rateIndex = <?php echo count($edit_rates); ?>;
    
    // Função para atualizar índices dos campos hidden monolíngue
    function updateMonoIndexes() {
        const rows = container.querySelectorAll('.rate-row');
        rows.forEach((row, idx) => {
            const monoInput = row.querySelector('.mono-val');
            if (monoInput) {
                monoInput.name = `rates_is_monolingual[${idx}]`;
            }
        });
    }
    ...
    document.getElementById('add_rate_btn').addEventListener('click', () => {
        ...
        // Atualizar o índice do campo hidden monolíngue
        const monoInput = clone.querySelector('.mono-val');
        if (monoInput) {
            monoInput.name = `rates_is_monolingual[${rateIndex}]`;
        }
    ...
    container.addEventListener('change', function(e) {
        ...
        if (target.classList.contains('mono-check')) {
            ...
            const groupTo = row.querySelector('.group-to');
            const labelTo = row.querySelector('.label-to');
            
            if (target.checked) {
                groupFrom.style.display = 'none';
                labelTo.textContent = 'Idioma (opcional)';
            } else {
