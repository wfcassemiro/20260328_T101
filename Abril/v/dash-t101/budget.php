...
if (!isset($_SESSION['budget_interpretation_expenses'])) $_SESSION['budget_interpretation_expenses'] = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    try {
        ...
        elseif ($act === 'add_interpretation_expense') {
            $exp = [
                'type' => $_POST['exp_type'] ?? 'travel',
                'description' => trim($_POST['exp_description'] ?? ''),
                'amount' => parseBRLFloat($_POST['exp_amount'] ?? '0'),
                'paid_by' => $_POST['exp_paid_by'] ?? 'client'
            ];
            $_SESSION['budget_interpretation_expenses'][] = $exp;
            $res = ['success' => true, 'expenses' => $_SESSION['budget_interpretation_expenses']];
        }

        elseif ($act === 'update_interpretation_expense') {
            $idx = (int)$_POST['exp_index'];
            if (isset($_SESSION['budget_interpretation_expenses'][$idx])) {
                $_SESSION['budget_interpretation_expenses'][$idx] = [
                    'type' => $_POST['exp_type'] ?? 'travel',
                    'description' => trim($_POST['exp_description'] ?? ''),
                    'amount' => parseBRLFloat($_POST['exp_amount'] ?? '0'),
                    'paid_by' => $_POST['exp_paid_by'] ?? 'client'
        ...
        elseif ($act === 'remove_interpretation_expense') {
            $idx = (int)$_POST['exp_index'];
            array_splice($_SESSION['budget_interpretation_expenses'], $idx, 1);
            $res = ['success' => true, 'expenses' => $_SESSION['budget_interpretation_expenses']];
        }

        elseif ($act === 'get_interpretation_expenses') {
            $res = ['success' => true, 'expenses' => $_SESSION['budget_interpretation_expenses'] ?? []];
        }

        elseif ($act === 'add_custom_expense_type') {
            // Salvar tipos customizados de despesa na sessão
            $newType = trim($_POST['new_expense_type'] ?? '');
            if (!$newType) throw new Exception("Nome do tipo obrigatório.");
            if (!isset($_SESSION['budget_custom_expense_types'])) $_SESSION['budget_custom_expense_types'] = [];
            if (!in_array($newType, $_SESSION['budget_custom_expense_types'])) {
                $_SESSION['budget_custom_expense_types'][] = $newType;
            }
            $res = ['success' => true, 'type_key' => 'custom_' . count($_SESSION['budget_custom_expense_types']), 'type_label' => $newType];
        }
        ...
        elseif ($act === 'calculate') {
            ...
            // Somar despesas de interpretação do Cliente ao total final
            $expClientTotal = 0;
            $expInterpreterTotal = 0;
            foreach ($_SESSION['budget_interpretation_expenses'] ?? [] as $exp) {
                if ($exp['paid_by'] === 'client') {
                    $expClientTotal += (float)$exp['amount'];
                } else {
                    $expInterpreterTotal += (float)$exp['amount'];
                }
            }
            $final += $expClientTotal;
            ...
            $res = ['success' => true, 'results' => [
                ...
                'expClientTotal' => ceilTo2($expClientTotal),
                'expInterpreterTotal' => ceilTo2($expInterpreterTotal)
            ]];
        }
...
if (isset($_GET['clear'])) {
    unset($_SESSION['analyses'], $_SESSION['budget_client'], $_SESSION['budget_costs'], $_SESSION['budget_flow_step'], $_SESSION['budget_params'], $_SESSION['wc_weights'], $_SESSION['budget_interpretation_expenses'], $_SESSION['budget_custom_expense_types']);
...
/* ========= MODAL DESPESAS DE INTERPRETAÇÃO ========= */
.interp-expenses-card { border-color: rgba(255, 152, 0, 0.3) !important; }
.interp-expenses-card h2 { background: linear-gradient(90deg, rgba(255, 152, 0, 0.2), transparent) !important; }

.expenses-table { width: 100%; border-collapse: collapse; }
.expenses-table th { text-align: left; padding: 10px 8px; font-size: 0.78rem; color: #94a3b8; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1px solid rgba(255,255,255,0.1); }
.expenses-table td { padding: 8px; vertical-align: middle; }
.expenses-table .vision-input, .expenses-table .vision-select { height: 38px; padding: 0 10px; font-size: 0.85rem; min-width: 90px; }

.expense-row { animation: fadeIn 0.3s; }
.expense-row:hover { background: rgba(255,255,255,0.02); }

.btn-remove-expense { width: 32px; height: 32px; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); color: #ef4444; border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: 0.2s; }
.btn-remove-expense:hover { background: #ef4444; color: #fff; }

.paid-by-toggle { display: flex; border-radius: 8px; overflow: hidden; border: 1px solid rgba(255,255,255,0.15); }
.paid-by-toggle label { padding: 6px 12px; font-size: 0.78rem; cursor: pointer; color: #94a3b8; background: rgba(0,0,0,0.2); transition: 0.2s; display: flex; align-items: center; white-space: nowrap; margin: 0; }
.paid-by-toggle input { display: none; }
.paid-by-toggle input[value="client"]:checked + label { background: rgba(16, 185, 129, 0.25); color: #6ee7b7; font-weight: 600; }
.paid-by-toggle input[value="interpreter"]:checked + label { background: rgba(245, 158, 11, 0.25); color: #fbbf24; font-weight: 600; }

.expenses-summary-box { background: rgba(0,0,0,0.2); border-radius: 10px; padding: 12px 16px; margin-top: 12px; }
.expenses-summary-row { display: flex; justify-content: space-between; padding: 3px 0; color: #94a3b8; font-size: 0.88rem; }
.expenses-summary-row.total { border-top: 1px solid rgba(255,255,255,0.1); padding-top: 8px; margin-top: 4px; font-weight: 700; color: #fff; }

.btn-interp-expenses { display: none; margin-top: 15px; padding: 12px 20px; font-size: 0.9rem; background: rgba(255, 152, 0, 0.12); border: 1px dashed rgba(255, 152, 0, 0.35); color: #fbbf24; border-radius: 10px; cursor: pointer; transition: 0.2s; width: 100%; text-align: center; }
.btn-interp-expenses.visible { display: flex; align-items: center; justify-content: center; gap: 8px; }
.btn-interp-expenses:hover { background: rgba(255, 152, 0, 0.25); border-style: solid; }
.btn-interp-expenses .exp-badge { background: #f59e0b; color: #000; font-size: 0.7rem; font-weight: 700; padding: 1px 6px; border-radius: 10px; }
...
.btn-add-exp-type:hover { background: #8b5cf6; border-color: #8b5cf6; }
</style>
...
<div class="main-content">
    ...
    <div class="video-card <?= ($isRevisao && $step >= 2) || $step >= 4 ? '' : ($step < 4 && !$isRevisao ? 'disabled' : '') ?>" id="cardCosts">
        ...
        <div class="vision-form-refined">
            ...
            $interpExpenses = $_SESSION['budget_interpretation_expenses'] ?? [];
            $interpExpCount = count(array_filter($interpExpenses, function($e) { return $e['amount'] > 0; }));
            ?>
            <button type="button" id="btnInterpExpenses" class="btn-interp-expenses <?= $isInterpretacao ? 'visible' : '' ?>" onclick="openModal('modalInterpExpenses')">
                <i class="fas fa-receipt"></i> Incluir despesas da interpretação
                <?php if ($interpExpCount > 0): ?>
                    <span class="exp-badge"><?= $interpExpCount ?></span>
                <?php endif; ?>
            </button>
...
<!-- MODAL: DESPESAS DE INTERPRETAÇÃO -->
<div id="modalInterpExpenses" class="vision-modal">
    <div class="vision-modal-content" style="max-width:750px;">
        <div class="modal-header">
            <h3><i class="fas fa-receipt" style="color:#fbbf24;"></i> Despesas de Interpretação</h3>
            <button class="btn-close" onclick="closeModal('modalInterpExpenses')">&times;</button>
        </div>
        <p style="color:#94a3b8; font-size:0.85rem; margin-top:-10px; margin-bottom:15px;">
            Despesas do <strong style="color:#6ee7b7;">Cliente</strong> somam ao faturamento.
            Despesas do <strong style="color:#fbbf24;">Intérprete</strong> são controle interno.
        </p>
        ...
        <div style="overflow-x:auto;">
            <table class="expenses-table">
                ...
                <tbody id="interpExpensesBody"></tbody>
            </table>
        </div>

        <button type="button" class="vision-btn vision-btn-secondary" id="btnAddInterpExpense" style="margin-top:10px;">
            <i class="fas fa-plus-circle"></i> Adicionar Despesa
        </button>

        <div class="expenses-summary-box" id="interpExpSummary">
            <div class="expenses-summary-row"><span style="color:#6ee7b7;">Cliente (faturamento):</span><span id="interpSumClient">0,00</span></div>
            <div class="expenses-summary-row"><span style="color:#fbbf24;">Intérprete (controle):</span><span id="interpSumInterp">0,00</span></div>
            <div class="expenses-summary-row total"><span>Total Despesas:</span><span id="interpSumTotal">0,00</span></div>
        </div>

        <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:20px;">
            <button type="button" class="vision-btn vision-btn-secondary" onclick="closeModal('modalInterpExpenses')">Fechar</button>
            <button type="button" class="vision-btn vision-btn-success" id="btnConfirmInterpExpenses"><i class="fas fa-check"></i> Confirmar</button>
        </div>
    </div>
</div>

<!-- MODAL: NOVO TIPO DE DESPESA -->
<div id="modalNewExpenseType" class="vision-modal">
    <div class="vision-modal-content" style="max-width:400px;">
        <div class="modal-header">
            <h3>Novo tipo de despesa</h3>
            <button class="btn-close" onclick="closeModal('modalNewExpenseType')">&times;</button>
        </div>
        <div class="form-group">
        ...
        <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:15px;">
            <button type="button" class="vision-btn vision-btn-secondary" onclick="closeModal('modalNewExpenseType')">Cancelar</button>
            <button type="button" class="vision-btn vision-btn-primary" id="btnSaveNewExpenseType">Adicionar</button>
        </div>
    </div>
</div>
...
// =============================================
let interpExpenses = <?= json_encode($_SESSION['budget_interpretation_expenses'] ?? []) ?>;
const defaultExpTypes = {
    'travel': 'Viagem', 'accommodation': 'Hospedagem', 'food': 'Alimentação', 'equipment': 'Equipamento'
};
let customExpTypes = <?= json_encode($_SESSION['budget_custom_expense_types'] ?? []) ?>;

// Mostrar/ocultar botão quando muda o serviço
const selService = document.getElementById('selService');
if (selService) {
    const origChange = selService.onchange;
    selService.addEventListener('change', function() {
        const btn = document.getElementById('btnInterpExpenses');
        if (this.value.toLowerCase().includes('interpret')) {
...
function getExpTypeLabel(key) {
    if (defaultExpTypes[key]) return defaultExpTypes[key];
    // Verificar customizados (formato: custom_N -> label é o valor em customExpTypes)
    if (key.startsWith('custom_')) {
        const idx = parseInt(key.replace('custom_', '')) - 1;
        return customExpTypes[idx] || key;
    }
    return key;
}

function buildExpTypeOptions(selected) {
    let html = '';
    for (const [key, label] of Object.entries(defaultExpTypes)) {
        html += '<option value="' + key + '"' + (selected === key ? ' selected' : '') + '>' + label + '</option>';
    }
    customExpTypes.forEach((label, idx) => {
        const key = 'custom_' + (idx + 1);
        html += '<option value="' + key + '"' + (selected === key ? ' selected' : '') + '>' + escHtml(label) + '</option>';
    });
    return html;
}

function renderInterpExpenses() {
    const tbody = document.getElementById('interpExpensesBody');
    if (!interpExpenses.length) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:#64748b; padding:20px;">Nenhuma despesa. Clique em "Adicionar Despesa".</td></tr>';
        return;
    }
    tbody.innerHTML = '';
    interpExpenses.forEach((exp, idx) => {
        const clientChk = exp.paid_by === 'client' ? 'checked' : '';
        const interpChk = exp.paid_by === 'interpreter' ? 'checked' : '';
        const amtVal = exp.amount > 0 ? fmtNum(exp.amount) : '';
        
        const tr = document.createElement('tr');
        tr.className = 'expense-row';
        tr.innerHTML = 
            '<td><div class="input-with-plus"><select class="vision-select exp-type-sel" data-idx="' + idx + '">' + buildExpTypeOptions(exp.type) + '</select>' +
            '<button type="button" class="btn-add-exp-type" title="Novo tipo" onclick="openNewExpTypeModal(' + idx + ')">+</button></div></td>' +
            '<td><input type="text" class="vision-input exp-desc-inp" data-idx="' + idx + '" value="' + escHtml(exp.description) + '" placeholder="Descrição"></td>' +
            '<td><input type="text" class="vision-input exp-amt-inp" data-idx="' + idx + '" value="' + amtVal + '" placeholder="0,00"></td>' +
            '<td><div class="paid-by-toggle">' +
                '<input type="radio" name="exp_pb_' + idx + '" value="client" id="epc_' + idx + '" ' + clientChk + '><label for="epc_' + idx + '">Cliente</label>' +
                '<input type="radio" name="exp_pb_' + idx + '" value="interpreter" id="epi_' + idx + '" ' + interpChk + '><label for="epi_' + idx + '">Intérprete</label>' +
            '</div></td>' +
            '<td><button type="button" class="btn-remove-expense" onclick="removeInterpExpense(' + idx + ')"><i class="fas fa-times"></i></button></td>';
        tbody.appendChild(tr);
    });
}

function syncInterpExpFromUI() {
    document.querySelectorAll('.expense-row').forEach((row, idx) => {
        if (interpExpenses[idx]) {
            const typeSel = row.querySelector('.exp-type-sel');
            const descInp = row.querySelector('.exp-desc-inp');
            const amtInp = row.querySelector('.exp-amt-inp');
            const paidBy = row.querySelector('input[name="exp_pb_' + idx + '"]:checked');
            
            interpExpenses[idx].type = typeSel ? typeSel.value : 'travel';
            interpExpenses[idx].description = descInp ? descInp.value : '';
            interpExpenses[idx].amount = amtInp ? (parseFloat(amtInp.value.replace(/\./g, '').replace(',', '.')) || 0) : 0;
            interpExpenses[idx].paid_by = paidBy ? paidBy.value : 'client';
        }
    });
}

function updateInterpExpSummary() {
    syncInterpExpFromUI();
    let clientT = 0, interpT = 0;
    interpExpenses.forEach(e => {
        if (e.paid_by === 'client') clientT += e.amount;
        else interpT += e.amount;
    });
    document.getElementById('interpSumClient').textContent = fmtNum(clientT);
    document.getElementById('interpSumInterp').textContent = fmtNum(interpT);
    document.getElementById('interpSumTotal').textContent = fmtNum(clientT + interpT);
}

// Delegação de eventos no tbody
document.getElementById('interpExpensesBody').addEventListener('input', updateInterpExpSummary);
document.getElementById('interpExpensesBody').addEventListener('change', updateInterpExpSummary);

document.getElementById('btnAddInterpExpense').addEventListener('click', () => {
    interpExpenses.push({ type: 'travel', description: '', amount: 0, paid_by: 'client' });
    renderInterpExpenses();
    updateInterpExpSummary();
});

window.removeInterpExpense = function(idx) {
    interpExpenses.splice(idx, 1);
    renderInterpExpenses();
    updateInterpExpSummary();
};

// Confirmar despesas: salvar na sessão via AJAX
document.getElementById('btnConfirmInterpExpenses').addEventListener('click', async () => {
    syncInterpExpFromUI();
    
    // Limpar sessão e re-adicionar
    await ajax({ ajax_action: 'get_interpretation_expenses' }); // dummy para inicializar
    
    // Remover todas existentes primeiro
    const current = await ajax({ ajax_action: 'get_interpretation_expenses' });
    if (current.expenses) {
        for (let i = current.expenses.length - 1; i >= 0; i--) {
            await ajax({ ajax_action: 'remove_interpretation_expense', exp_index: i });
        }
    }
    
    // Adicionar novas
    for (const exp of interpExpenses) {
        if (exp.amount > 0) {
            await ajax({
                ajax_action: 'add_interpretation_expense',
                exp_type: exp.type,
                exp_description: exp.description,
                exp_amount: exp.amount.toFixed(2).replace('.', ','),
                exp_paid_by: exp.paid_by
            });
        }
    }
    ...
    const btn = document.getElementById('btnInterpExpenses');
    const validCount = interpExpenses.filter(e => e.amount > 0).length;
    let badge = btn.querySelector('.exp-badge');
    if (validCount > 0) {
        if (!badge) {
    ...
    closeModal('modalInterpExpenses');
});

// Abrir modal: renderizar estado atual
const origOpenModal = window.openModal;
const _origOpenModalFn = window.openModal;
window.openModal = function(id) {
    _origOpenModalFn(id);
    if (id === 'modalInterpExpenses') {
        renderInterpExpenses();
        updateInterpExpSummary();
    }
};
...
window.openNewExpTypeModal = function(idx) {
    ...
    openModal('modalNewExpenseType');
    setTimeout(() => document.getElementById('newExpenseTypeName').focus(), 100);
};

document.getElementById('btnSaveNewExpenseType').addEventListener('click', async () => {
    const name = document.getElementById('newExpenseTypeName').value.trim();
    if (!name) return alert('Digite o nome do tipo.');
    
    const r = await ajax({ ajax_action: 'add_custom_expense_type', new_expense_type: name });
    if (r.success) {
        customExpTypes.push(name);
        // Atualizar o select da linha
        if (pendingExpTypeIdx !== null && interpExpenses[pendingExpTypeIdx]) {
            interpExpenses[pendingExpTypeIdx].type = r.type_key;
        }
        renderInterpExpenses();
        closeModal('modalNewExpenseType');
    } else {
