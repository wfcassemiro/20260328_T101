...
// --- Busca (ATUALIZADO: inclui campo de observações) ---
...
function getWhatsAppLink($phone, $countryCode = '+55') {
    // Remove caracteres não numéricos do telefone
    $cleanPhone = preg_replace('/[^0-9]/', '', $phone);
    // Remove o + do código do país
    $cleanCode = preg_replace('/[^0-9]/', '', $countryCode);
         ...
         <form method="GET" class="vision-form-refined" action="freelancers_list.php" style="padding: 25px 30px 30px;">
            <div class="form-row" style="grid-template-columns: 1fr auto; margin-bottom: 0;">
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    <input type="text" id="search_term" name="search_term" class="vision-input" value="<?php echo htmlspecialchars($search_term); ?>" placeholder="Nome, e-mail, serviço, idioma, observações...">
                </div>
                                    ...
                                    <div class="contact-info">
                                        <?php if ($freelancer['email']): ?>
                                            <div class="contact-row">
                                                ...
                                                <button type="button" class="btn-copy-email" onclick="copyToClipboard('<?php echo htmlspecialchars($freelancer['email']); ?>', this)" title="Copiar e-mail">
                                        ...
                                        <?php if ($freelancer['phone']): ?>
                                            <div class="contact-row phone-row">
                                                <span class="po-number-refined">
                                                    <i class="fas fa-phone-alt" style="font-size:0.7rem;"></i> 
                                                    <?php echo htmlspecialchars(($freelancer['phone_country_code'] ?? '+55') . ' ' . $freelancer['phone']); ?>
                                                </span>
                                                <?php if (!empty($freelancer['is_whatsapp'])): ?>
                                                    <a href="<?php echo getWhatsAppLink($freelancer['phone'], $freelancer['phone_country_code'] ?? '+55'); ?>" target="_blank" class="btn-whatsapp" title="Abrir WhatsApp">
...
.vision-btn-success { background: #22c55e; border-color: #22c55e; }
.vision-btn-success:hover { background: #16a34a; border-color: #16a34a; }
...
.po-number-refined { font-size: 0.8rem; color: var(--text-muted); background: rgba(255, 255, 255, 0.05); padding: 3px 8px; border-radius: 8px; display: inline-flex; align-items: center; gap: 5px; }
...
// Função para copiar e-mail para área de transferência
